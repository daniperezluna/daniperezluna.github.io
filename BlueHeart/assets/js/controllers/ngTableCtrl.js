...
app.controller('ngTableCtrl', ["$scope", "$http", "$timeout", "ngTableParams", function ($scope, $http, $timeout, ngTableParams) {
    
    var url = "http://blueheart.16mb.com/blueheart/services/"; 


     $scope.promise = $http.get(url+"leerDatos.php")
...
app.controller('ngTableCtrl2', ["$scope", "$filter", "ngTableParams", function ($scope, $filter, ngTableParams) {
    $scope.tableParams = new ngTableParams({
        ...
        getData: function ($defer, params) {
            ...
            var orderedData = params.sorting() ? $filter('orderBy')($scope.datosRec, params.orderBy()) : $scope.datosRec;
...
app.controller('ngTableCtrl3', ["$scope", "$filter", "ngTableParams", function ($scope, $filter, ngTableParams) {
    $scope.tableParams = new ngTableParams({
        ...
        getData: function ($defer, params) {
            // use build-in angular filter
            var orderedData = params.filter() ? $filter('filter')($scope.datosRec, params.filter()) : $scope.datosRec;
